...
With verification, explainability, and data recording features

Four binary classifiers:
1. AD vs CN (English) - 85.9%
2. AD vs CN (Spanish) - 85.1%
3. MCI vs CN (English) - 76.6%
4. MCI vs CN (Chinese) - 60%
"""
...
DATA_FILE = "Egyptians Data Results.csv"  # CSV file for recording results
...
MODEL_CONFIGS = {
    ...
    "AD (Spanish)": {
        "folder": "Spanish AD-CN 85.1%",
        "accuracy": "85.1%",
        "description": "Detección de Enfermedad de Alzheimer",
        "language": "es",
        "features": ["acoustic", "linguistic"],
        "condition": "Alzheimer's Disease"
    },
...
def get_next_participant_id():
    """Get the next participant ID (PAR_X)"""
    try:
        if os.path.exists(DATA_FILE):
            df = pd.read_csv(DATA_FILE)
            if len(df) > 0:
                # Extract numbers from existing IDs
                existing_ids = df['Identification'].str.extract(r'PAR_(\d+)')[0].astype(float)
                existing_ids = existing_ids.dropna()
        ...
        return f"PAR_{next_num}"
    except Exception as e:
        st.error(f"Error getting next ID: {e}")
        return "PAR_ERROR"

def record_participant_data(participant_id, risk_score):
    """Record participant data to CSV"""
    try:
        ...
        if os.path.exists(DATA_FILE):
            ...
            df = pd.DataFrame(columns=['Identification', 'Age', 'Gender', 'Risk Assessment'])
        
        # Create new row
        new_row = {
            'Identification': participant_id,
            'Age': '',  # Leave blank for manual filling
            'Gender': '',  # Leave blank for manual filling
            'Risk Assessment': f"{risk_score:.1f}%"
        }
        ...
        st.error(f"Error recording data: {e}")
        return False
...
def apply_custom_css():
    ...
    .record-button button:hover {
        background: linear-gradient(90deg, #218838 0%, #1fa884 100%) !important;
    }
    </style>
...
def extract_spanish_linguistic_features(transcript, audio_path):
    """
    Extract Spanish linguistic features - EXACT MATCH TO TRAINING
    Uses es_core_news_sm for accurate Spanish NLP
    ...
    try:
        ...
        # Fallback to English model if Spanish not available
    ...
    # ============================================
    # Verb diversity
    verbs = [token.lemma_.lower() for token in tokens if token.pos_ in ['VERB', 'AUX']]
    unique_verbs = len(set(verbs))
    verb_diversity = unique_verbs / len(verbs) if len(verbs) > 0 else 0
    
    # Subjunctive mood count
    subjunctive_count = sum(1 for token in tokens 
                           if token.pos_ == 'VERB' and 'Sub' in token.morph.get('Mood', []))
    
    # Clitics (me, te, se, lo, la, le, nos, os, los, las, les)
    spanish_clitics = {'me', 'te', 'se', 'lo', 'la', 'le', 'nos', 'os', 'los', 'las', 'les'}
    clitic_count = sum(1 for word in words if word in spanish_clitics)
    pct_clitics = (clitic_count / total_words * 100) if total_words > 0 else 0
    
    # Prepositional variety
    prepositions = [token.text.lower() for token in tokens if token.pos_ == 'ADP']
    unique_preps = len(set(prepositions))
    prep_variety = unique_preps / len(prepositions) if len(prepositions) > 0 else 0
...
def analyze_features(ling_dict, risk_score, language="en"):
    ...
    if language in ["en", "es"]:  # English and Spanish use same core features
        ...
        # Spanish-specific features (if present)
        if language == "es":
            verb_div = ling_dict.get('verb_diversity', 0)
            if verb_div and verb_div < 0.3:
                risk_factors.append(f"Limited verb variety ({verb_div:.3f}) - repetitive language")
            elif verb_div and verb_div > 0.5:
                positive_factors.append(f"Good verb variety ({verb_div:.3f})")
            
            subj_count = ling_dict.get('subjunctive_count', 0)
            if subj_count and subj_count > 2:
                positive_factors.append(f"Uses complex grammar (subjunctive mood: {subj_count})")
            elif subj_count == 0:
                findings.append("No complex grammatical structures (subjunctive)")
...
def main():
                ...
                try:
                    ...
                    try:
                        if config["language"] == "en":
                            ...
                            ling_dict = extract_spanish_linguistic_features(transcript, audio_file)
                        else:  # Chinese
                    ...
                    # Store in session state for recording
                    ...
                    st.markdown("---")
                    st.markdown("### 💾 Record Participant Data")
                    
                    col_record1, col_record2 = st.columns([3, 1])
                    
                    with col_record1:
                        st.info("📋 Click below to save this result to the research database. " +
                               "A new participant ID will be automatically assigned.")
                    
                    with col_record2:
                        if st.button("📝 Record Data", key="record_btn", use_container_width=True):
                            next_id = get_next_participant_id()
                            
                            if record_participant_data(next_id, risk_score):
                                st.success(f"✅ Data recorded as **{next_id}** with risk score **{risk_score:.1f}%**")
                                st.info("💡 Remember to fill in Age and Gender in the CSV file manually")
                            else:
                                st.error("❌ Failed to record data. Check file permissions.")
                    ...
                    risk_factors, findings, positive_factors = analyze_features(
                        ling_dict, risk_score, config["language"]
                    )
                    ...
                    with st.expander("🔢 Raw Feature Values"):
                        ...
                        with ind_col2:
                            ...
                            if config["language"] in ["en", "es"] and 'MATTR' in ling_dict:
    ...
    <div class='footer'>
        ...
        <p style='font-size: 0.75rem; margin-top: 1rem;'>
            Models: AD-CN English (85.9%) | AD-CN Spanish (85.1%) | MCI-CN English (76.6%) | MCI-CN Chinese (60%)<br>
